...
class StockRNN(object):
    ...
    def __init__(self, seq_size=200, input_dimension=6, hidden_layer_size=128):
    ...
    def train_pred_rnn(self):
        ...
        class_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
            labels=tf.cast(self.Y > 0, tf.float32), logits=y_hat))

        total_loss = loss + class_loss*0.1
        train_optim = tf.train.AdamOptimizer(learning_rate=0.001).minimize(total_loss)
        ...
        with tf.Session() as sess:
            ...
            writer = pd.ExcelWriter('lstm_pred.xlsx')
